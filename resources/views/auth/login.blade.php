...
<div class="relative min-h-screen flex items-center justify-center bg-mint-cream py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8 bg-white shadow-lg rounded-lg p-10 relative z-10">
        ...
        <form class="mt-8 space-y-6" method="POST" action="{{ route('login') }}">
            ...
            <div class="rounded-md shadow-sm -space-y-px">
                <div>
                    ...
                    <input id="email" name="email" type="email" autocomplete="email" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 focus:z-10 sm:text-sm @error('email') border-red-500 @enderror" placeholder="Email address" value="{{ old('email') }}">
                    @error('email')
        ...
        <div class="mt-6 text-center">
            <p class="text-sm text-gray-600">{{ __('No account?') }} <a href="{{ route('register') }}" class="font-medium text-blue-600 hover:text-blue-500">{{ __('Register') }}</a></p>
        </div>
    </div>
</div>
@endsection
