...
<head>
    ...
    <title>@yield('title', 'Apple Pie or Cheesecake')</title>
    <link href="{{ asset('website.css') }}" rel="stylesheet">
    <script type="module" src="{{ asset('website.js') }}"></script>
</head>

<body class="bg-mint-cream flex flex-col h-screen">
    <header class="p-4 bg-feldgrau shadow-md flex justify-between items-center">
        <img src="{{ asset('/images/appleicons.png') }}" alt="Icon" class="h-10 w-10">
        <section class="text-sm font-bold text-white">Apple Pie or Cheesecake</section>
        <button id="sidebar-toggle" aria-label="Toggle sidebar" aria-expanded="false" class="text-2xl sm:text-4xl text-white" onclick="toggleSidebar()">&#9776;</button>
    </header>
            ...
            <section class="flex flex-col h-full justify-between">
                <section>
                    <section class="flex justify-between items-center px-4 py-2">
                        <h3 class="text-lg sm:text-xl font-semibold text-feldgrau">Menu</h3>
                        <button class="text-2xl text-feldgrau" onclick="menu_close()">&#10005;</button>
                    </section>
                    <!-- Sidebar content -->
