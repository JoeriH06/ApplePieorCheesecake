...
@section('content')
    ...
    <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <article class="p-4 bg-mint-cream shadow-md rounded">
            ...
            <a href="{{ route('recipes.show', ['id' => $recipe->id]) }}" class="text-ash-gray hover:underline">Read More</a>
            <section class="mt-2 flex space-x-2">
                <a href="{{ route('recipes.edit', ['id' => $recipe->id]) }}" class="bg-ash-gray text-feldgrau py-1 px-2 rounded">Edit</a>
                <form action="{{ route('recipes.destroy', ['id' => $recipe->id]) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                    @csrf
                    @method('DELETE')
